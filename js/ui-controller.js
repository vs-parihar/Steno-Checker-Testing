...
function resetData(){$('tx').value='';$('stTitle').innerText='No Audio';clnAu();$('wc').value=0;$('st').innerText='Ready';$('trI').value='';$('trI').disabled=0;$('resBox').style.display='none';$('checkV').innerHTML='';$('eL').innerHTML='';upWC();if(is)au.pause();is=0;updatePlBtn();crpS=0;crpE=0;origTxt='';$('cfBox').style.display='none'}
...
function upExD(){const s=$('exSel').value;let e=exams[s]||exams.custom;
if(s!=='custom'){$('cWpm').value=e.wpm;$('cDur').value=e.dur;$('cLg').value=e.rules.wc;$('cCap').value=e.rules.cap;$('cCom').value=e.rules.com;$('cPun').value=e.rules.pun;$('cSpl').value=e.rules.spl;$('cSub').value=e.rules.sub;$('cMaxE').value=e.rules.max}
$('exDesc').innerText=`Preset: ${e.name}`;
}

function selEx(){const s=$('exSel').value;exMode=$('exMode').value;
exRules={cap:$('cCap').value,com:$('cCom').value,pun:$('cPun').value,spl:$('cSpl').value,sub:$('cSub').value,wc:$('cLg').value,max:$('cMaxE').value};
if(s==='custom'){exams.custom.wpm=$('cWpm').value;exams.custom.dur=$('cDur').value}
curEx=s;$('lg').value=exRules.wc;$('exM').classList.remove('active');$('srcM').classList.add('active');
const trV = (s!=='custom'?exams[s].trans:0); if(trV > 0) {$('trT').value = trV; $('trC').value=trV;}}
...
async function lA(i,idx){curI=idx;curO=i;openM();$('tx').value="Loading...";const v=gW(i);if(v){$('wc').value=v;$('st').innerText='W: '+v;if(i.dur){$('wi').value=(v/(i.dur/60)).toFixed(2);$('ri').value="1.0000";sync('r')}}const id=i.url?.split('details/')[1],dl=`https://archive.org/download/${id}/`;let auSrc=i.audio;try{if(id){const mr=await fetch(`https://archive.org/metadata/${id}`),md=await mr.json(),fs=md.files||[];const bf=fs.find(f=>f.name.endsWith('.wav'))||fs.find(f=>f.name.endsWith('.flac'))||fs.find(f=>f.name.endsWith('.m4a'))||fs.find(f=>f.name.endsWith('.mp3'));if(bf)auSrc=dl+bf.name;if(i.matter){const r=await fetch(i.matter);$('tx').value=await r.text()}else if(fs.find(f=>f.name.endsWith('.txt'))){const r=await fetch(dl+fs.find(f=>f.name.endsWith('.txt')).name);$('tx').value=await r.text()}else{$('tx').value=md.metadata.description?.replace(/<[^>]*>?/gm,'')||"No text"}}}catch(e){if(!i.matter)$('tx').value="Text Load Fail"}clnAu();au.src=auSrc;au.load();meta(i.title);origTxt=$('tx').value;upWC();if(exMode==='mock')prepMock()}
