...
function resetData() {
    $('tx').value = ''; $('stTitle').innerText = 'No Audio'; clnAu(); $('wc').value = 0; $('st').innerText = 'Ready'; $('trI').value = ''; $('trI').disabled = 0; $('resBox').style.display = 'none'; $('rS').innerHTML = ''; $('checkV').innerHTML = ''; $('eL').innerHTML = ''; upWC(); if (is) au.pause(); is = 0; updatePlBtn(); crpS = 0; crpE = 0; origTxt = ''
}

function updateStatus() {
    $('stWPM').textContent = $('wi').value + ' WPM'; $('stRate').textContent = $('ri').value + 'x'; $('stWC').textContent = $('wc').value + ' W'; $('stMode').textContent = exMode === 'mock' ? 'Mock Mode' : 'Practice'; $('stMode').style.color = exMode === 'mock' ? '#ef4444' : 'inherit'
}
...
function renT() {
    const t = $('tags'), set = new Set(); curL.forEach(i => i.tags.forEach(g => set.add(g))); t.innerHTML = '';
    Array.from(set).sort().forEach(g => { let s = document.createElement('span'); s.className = 'filter-chip'; s.textContent = g; s.onclick = () => { s.classList.toggle('active'); renL() }; t.appendChild(s) })
}
...
function renL() {
    ...
    if (so === 'sl') filtL.sort((a, b) => gW(a) - gW(b)); if (so === 'ls') filtL.sort((a, b) => gW(b) - gW(a));
    if (so === 'sh') { for (let i = filtL.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [filtL[i], filtL[j]] = [filtL[j], filtL[i]] } }
    $('ls').innerHTML = ''; filtL.forEach((i, idx) => { let d = document.createElement('div'); d.className = 'list-item'; d.innerHTML = `<div style="font-weight:500">${i.title}</div><div style="font-size:9px;opacity:0.7">${gW(i)} words • ${fT(i.dur || 0)}</div>`; d.onclick = () => lA(i, idx); $('ls').appendChild(d) })
}

async function lA(i, idx) {
    curI = idx; curO = i; openM(); $('tx').value = "Loading..."; const v = gW(i);
    if (v) { $('wc').value = v; $('st').innerText = 'W: ' + v; if (i.dur) { $('wi').value = (v / (i.dur / 60)).toFixed(2); $('ri').value = "1.0000"; sync('r') } }
