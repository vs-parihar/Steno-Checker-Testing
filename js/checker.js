...
algn=[];let i=m,j=n;while(i>0||j>0){const op=p[i][j];if(op===0){const c=cmpW(oToks[i-1].t,uToks[j-1].t),raw=oToks[i-1].t!==uToks[j-1].t;algn.unshift({t:c===0?(raw?'p':'k'):'w',o:oToks[i-1],u:uToks[j-1],i:i-1,j:j-1,w:c});i--;j--}
...
if(e.t==='f'){v=r.sub==='d'?1:0.5}else if(e.t==='m'){v=1}
    ...
    const diff=[...e.o.t].filter(c=>!e.u.t.includes(c)).concat([...e.u.t].filter(c=>!e.o.t.includes(c))).join('');
    if(diff.match(/[.?!|\u0964]/))v=parseFloat(r.pun);
    else if(diff.match(/[,]/))v=parseFloat(r.com);
    else v=0;
}
else if(e.t==='w'){if(e.w>=1){v=1}else{v=0;if(parseFloat(r.cap)>0&&e.o.t[0]!==e.u.t[0])v=Math.min(1,Math.max(v,parseFloat(r.cap)))}}}
...
else if(x.t==='p'){s.className='c-pun';s.textContent=x.u.t+' ';s.onclick=()=>jump(x.id)}
...
it.innerHTML=`<div class="row" style="margin:0"><span class="e-ctx">${getCtx(oToks,e.i,2,-1)}</span><span class="e-w ${e.val>=1?'m-err':(e.t==='f'?'c-ins':(e.t==='m'?'c-del':(e.t==='p'?'c-pun':'m-h')))}">${e.t==='m'?e.o.t:e.u.t}</span><span class="e-ctx">${getCtx(oToks,e.i,2,1)}</span><span style="font-size:9px;opacity:0.7;margin-left:auto">(-${e.val})</span></div>
