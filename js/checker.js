...
else if(e.t==='p'){const oP=e.o.t.match(/[.?!|\u0964,;]$/),uP=e.u.t.match(/[.?!|\u0964,;]$/);v=(oP&&!uP)?parseFloat(r.pun):0}
...
function renCheckV(){const v=$('checkV');v.innerHTML='';algn.forEach((x,k)=>{let s=document.createElement('span');s.id=`mk-${x.id}`;
...
else if(x.t==='p'){const oW=x.o.t.replace(/[.?!|\u0964,;]$/,''),oP=x.o.t.slice(oW.length);s.innerHTML=`${oW}<span class="c-del">${oP}</span> `;s.onclick=()=>jump(x.id)}
else{if(exRules.sub==='d'&&x.val>=1&&x.w>=1){s.innerHTML=`<span class="c-del">${x.o.t}</span> <span class="c-ins">${x.u.t}</span> `}else{s.className='c-grp';s.innerHTML=`<span class="${x.val>=1?'c-sub-f':'c-sub-h'}">${x.u.t}<span class="c-cor">(${x.o.t})</span></span> `}s.onclick=()=>jump(x.id)}
v.appendChild(s)})}
...
it.innerHTML=`<div class="row" style="margin:0"><span class="e-ctx">${getCtx(oToks,e.i,2,-1)}</span><span class="e-w ${e.val>=1?'m-err':(e.t==='f'?'c-ins':(e.t==='m'?'c-del':(e.t==='p'?'c-del':'m-h')))}">${e.t==='m'?e.o.t:e.u.t}</span><span class="e-ctx">${getCtx(oToks,e.i,2,1)}</span><span style="font-size:9px;opacity:0.7;margin-left:auto">(-${e.val})</span></div>
