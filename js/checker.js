...
else{const oB=bare(x.o.t),uB=bare(x.u.t);if(oB===uB){const pRe=/[^\w\u0900-\u097F\u0964|\-]/g,oP=x.o.t.match(pRe)||[],uP=x.u.t.match(pRe)||[];if(oP.join('')!==uP.join('')){const mO=x.o.t.replace(pRe,''),mP=x.o.t.match(pRe)?.[0]||'';s.className='c-ok';s.innerHTML=`${mO}<span class="c-del" style="font-weight:bold">${mP}</span> `;s.onclick=()=>jump(x.id);v.appendChild(s);return}}
