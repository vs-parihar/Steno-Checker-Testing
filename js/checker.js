...
clearInterval(trTi);trTi=setInterval(()=>{if(trStopW)trElap++;else trRem--;let t=trStopW?trElap:trRem,m=Math.floor(t/60),s=t%60;$('trCD').textContent=`${m}:${s<10?'0':''}${s}`;if(!trStopW&&trRem<=0)subTr()},1000);
}
function subTr(){
...
const hs={'में':'मै','मै':'में','की':'कि','कि':'की','है':'हैं','हैं':'है','उनकी':'उसकी','उसकी':'उनकी'};
for(let i=1;i<=m;i++)for(let j=1;j<=n;j++){
const bo=bare(oToks[i-1].t),bu=bare(uToks[j-1].t),po=oToks[i-1].p,pu=uToks[j-1].p;
dp[i][j]=(bo===bu||hs[bo]===bu||po===pu||(i<m&&bare(oToks[i-1].t+oToks[i].t)===bu))?dp[i-1][j-1]+1:Math.max(dp[i-1][j],dp[i][j-1]);
}
...
let ty=(oToks[i-1].t===uToks[j-1].t)?'w':'h';errs.unshift({t:ty,o:oToks[i-1],u:uToks[j-1],i:i-1,j:j-1});i--;j--;
}else if(i>1&&j>0&&bare(oToks[i-2].t+oToks[i-1].t)===bu){
errs.unshift({t:'h',o:oToks[i-2],u:uToks[j-1],i:i-2,j:j-1});errs.unshift({t:'x',o:oToks[i-1],i:i-1});i-=2;j--;
...
errs.forEach((e,k)=>{if(e.t==='x')return;let sp=document.createElement('span');sp.id=`u-${k}`;sp.onclick=()=>revClick('u',k);
let c=e.t==='f'?'c-ins':e.t==='m'?'c-mis':e.t==='h'?'c-sp':'w-n';sp.className=c;sp.textContent=(e.t==='m'?e.o.t:e.u.t)+' ';v.appendChild(sp)});
}
function reCalc(){
...
if(parseFloat(r.cap)>0&&e.o.t[0]!==e.u.t[0])tE+=parseFloat(r.cap);
else if(e.o.t.match(/[,]/)&&!e.u.t.match(/[,]/))tE+=parseFloat(r.com);
else if(e.o.t.match(/[.?!|]/)&&!e.u.t.match(/[.?!|]/))tE+=parseFloat(r.pun);
}});const acc=Math.max(0,100-(tE/tot*100)),pass=acc>=(100-parseFloat(r.max));
$('resBox').style.display='flex';$('resTit').innerHTML=`<span style="color:${pass?'var(--gr)':'var(--re)'}">${pass?'PASSED':'FAILED'}</span>`;
$('resDet').innerHTML=`Errors: <b>${tE.toFixed(1)}</b> / ${tot} (${(tE/tot*100).toFixed(1)}%)<br>Accuracy: <b>${acc.toFixed(1)}%</b>`;
const l=$('eL');l.innerHTML='';errs.forEach((e,k)=>{if(e.t==='w'||e.t==='x')return;
l.innerHTML+=`<div class="e-it" id="err-${k}"><div class="row" style="margin:0"><span class="e-ctx">${getCtx(oToks,e.i,2,-1)}</span><span class="e-w ${e.t==='f'?'m-err':(e.t==='h'?'m-h':'m-mis')}" onclick="jump(${k})">${e.t==='m'?e.o.t:e.u.t}</span><span class="e-ctx">${getCtx(oToks,e.i,2,1)}</span></div></div>`;
});
}
...
function jump(k){
const e=errs[k];if(!e)return;clrHl();if(e.i!==-1){[...Array(5).keys()].forEach(x=>$( `o-${e.i-2+x}`)?.classList.add('ctx-hl'));$(`o-${e.i}`)?.scrollIntoView({behavior:'smooth',block:'center'})}
$(`u-${k}`)?.scrollIntoView({behavior:'smooth',block:'center'});$(`err-${k}`)?.scrollIntoView({behavior:'smooth',block:'center'});
}
...
function refine(){$('rsV').style.display='none';$('trV').style.display='flex';$('trI').disabled=0;$('trI').focus();trIp=1}
