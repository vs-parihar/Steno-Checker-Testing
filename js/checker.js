...
else if(op===3){const c=cmpW(oToks[i-1].t,uToks[j-2].t+" "+uToks[j-1].t);algn.unshift({t:'h',o:oToks[i-1],u:{t:uToks[j-2].t+" "+uToks[j-1].t},i:i-1,j:j-2,msg:'Split',w:c});i--;j-=2}
else if(op===4){const c=cmpW(oToks[i-2].t+" "+oToks[i-1].t,uToks[j-1].t);algn.unshift({t:'h',o:{t:oToks[i-2].t+" "+oToks[i-1].t},u:uToks[j-1],i:i-2,j:j-1,msg:'Merge',w:c});i-=2;j--}}
...
else if(e.t==='h'){v=e.w>=1?1:parseFloat(r.spl)}
...
function renCheckV(){const v=$('checkV'),sd=exRules.sub==='d',br=s=>s.normalize('NFC').replace(/[\u200B-\u200D\uFEFF]/g,'').replace(/[^\w\u0900-\u097F]/g,'').toLowerCase();v.innerHTML='';algn.forEach((x,k)=>{let s=document.createElement('span');s.id=`mk-${x.id}`;
...
else{if(sd&&x.val>=1&&x.w>=1){s.innerHTML=`<span class="c-del">${x.o.t}</span> <span class="c-ins">${x.u.t}</span> `}else{s.className='c-grp';
if(br(x.o.t)===br(x.u.t)){let o=x.o.t,u=x.u.t,i=0,j=0;while(i<o.length&&i<u.length&&o[i]===u[i])i++;while(j<o.length-i&&j<u.length-i&&o[o.length-1-j]===u[u.length-1-j])j++;
let mO=o.slice(i,o.length-j),mU=u.slice(i,u.length-j);s.innerHTML=o.slice(0,i)+(mO?`<span style="color:var(--ye)">${mO}</span>`:'')+(mU?`<span style="color:var(--sky)">${mU}</span>`:'')+o.slice(o.length-j)+' '}else{s.innerHTML=`<span class="${x.val>=1?'c-sub-f':'c-sub-h'}">${x.u.t}<span class="c-cor">(${x.o.t})</span></span> `}}s.onclick=()=>jump(x.id)}
v.appendChild(s)})}
