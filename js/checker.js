...
errs=algn.filter(x=>x.t!=='k'||checkPun(x));errs.forEach((e,idx)=>e.id=idx);$('trV').style.display='none';$('rsV').style.display='flex';$('tx').style.visibility='visible';reCalc()}

function checkPun(e){const pO=getP(e.o.t),pU=getP(e.u.t);if(pO[1]!==pU[1]||pO[3]!==pU[3]){e.pun=1;return true}return false}
function getP(s){return s.match(/^([^\w\u0900-\u097F]*)(.*?)([^\w\u0900-\u097F]*)$/)||['','','',s]}
...
if(e.t==='w' && e.w>=1)v=1;else{v=0;if(e.t==='w'&&parseFloat(r.cap)>0&&e.o.t[0]!==e.u.t[0])v=Math.max(v,parseFloat(r.cap));
if(e.pun){const pO=getP(e.o.t),pU=getP(e.u.t),isC=(pO[1]+pO[3]).includes(',')||(pU[1]+pU[3]).includes(',');v=Math.max(v,parseFloat(isC?r.com:r.pun))}}
...
function renCheckV(){const v=$('checkV'),sd=exRules.sub==='d';v.innerHTML='';algn.forEach((x,k)=>{let s=document.createElement('span');s.id=`mk-${x.id||'ok'}`;
if(x.t==='k' && !x.pun){s.className='c-ok';s.textContent=x.o.t+' '}
...
else{if(x.pun && x.val===0){s.className='c-warn';s.textContent=x.u.t+' '}
...
it.innerHTML=`<div class="row" style="margin:0"><span class="e-ctx">${getCtx(oToks,e.i,2,-1)}</span><span class="e-w ${e.val>=1?'m-err':(e.t==='f'?'c-ins':(e.t==='m'?'c-del':(e.pun&&e.val===0?'c-warn':'m-h')))}">${e.t==='m'?e.o.t:e.u.t}</span><span class="e-ctx">${getCtx(oToks,e.i,2,1)}</span><span style="font-size:9px;opacity:0.7;margin-left:auto">(-${e.val})</span></div>
...
const el=$(`mk-${id}`) || $(`mk-ok`);if(el){el.scrollIntoView({behavior:'smooth',block:'center'});
