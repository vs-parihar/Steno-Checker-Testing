let mockState = 0, mockTimer = null;
...
function prepMock() {
    ...
    $('wi').value = wpm; sync('w'); const auD = au.duration, exD = dur * 60;
    if (exMode === 'mock' && auD > exD + 30) { $('crpM').classList.add('active'); }
    else { startMock(); }
}
...
function cropTxt(pos, cnt) {
    ...
    else if (pos === 'mid') { const mid = Math.floor(t.length / 2); sl = t.slice(mid - (cnt / 2), mid + (cnt / 2) + 20); }
...
function nextMockStep() {
    const e = exams[curEx]; clearInterval(mockTimer); mockState++;
    $('mSkipBtn').onclick = () => nextMockStep();
    if (mockState === 1) {
        $('mStT').innerText = 'Dictation'; $('mSub').innerText = 'Playing Audio...'; $('mSkipBtn').innerText = 'Finish Early';
        au.currentTime = crpS; au.play(); is = 1; updatePlBtn();
        if (crpE > 0) { au.ontimeupdate = () => { if (au.currentTime >= crpE) { au.pause(); beeps(); nextMockStep(); } }; }
        else { au.onended = () => nextMockStep(); }
        $('mTimer').innerText = '--:--';
    } else if (mockState === 2) {
        is = 0; updatePlBtn(); runMockTimer(15, 'Break / Gap', 'Prepare for reading...', () => nextMockStep());
    } else if (mockState === 3) {
        runMockTimer(e.read * 60, 'Reading Time', 'Read your notes...', () => nextMockStep());
    } else if (mockState === 4) {
        $('mockOV').style.display = 'none'; $('trM').classList.add('active'); resTr();
        $('trT').value = e.trans; $('trC').value = e.trans; $('trA').disabled = 1;
        let cd = 10; $('trInf').innerText = `Auto-start in ${cd}s`;
...
function runMockTimer(sec, t, s, cb) {
    $('mStT').innerText = t; $('mSub').innerText = s; $('mSkipBtn').innerText = 'Skip';
    let rem = sec; const upd = () => { const m = Math.floor(rem / 60), s = rem % 60; $('mTimer').innerText = `${m}:${s < 10 ? '0' : ''}${s}`; };
    upd();
    mockTimer = setInterval(() => { rem--; upd(); if (rem <= 0) { clearInterval(mockTimer); cb(); } }, 1000);
}